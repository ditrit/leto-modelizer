Feature: Test roundtrip of the application : create project files
  ...
  Scenario Outline: Create project redirect to models page with correct plugin and create project files and folders
    ...
    And  I wait 1 second
    Then I expect current url is '{{projectName}}/modelizer/text\?path=<plugin>/{{modelName}}'
    And  I expect '[data-cy="file-explorer"] [data-cy="folder_{{projectName}}"]' is '{{projectName}}'
    And  I expect '[data-cy="file-explorer"] [data-cy="folder_<plugin>"]' exists
    And  I expect '[data-cy="file-explorer"] [data-cy="folder_<plugin>/{{modelName}}"]' exists
