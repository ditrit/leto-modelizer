Feature: Test models page: model deletion
  ...
  Scenario: Delete model, create another model to check previous model files are deleted
    ...
    And  I expect '[data-cy="model-card_terrator-plugin-{{modelName}}"]' not exists
    ...
    And  I click on '[data-cy="create-model-form"] [data-cy="submit-button"]'
    Then I expect current url is 'modelizer/{{projectName}}/model\?path=terrator-plugin/{{modelName2}}'

    # Go to text view and check files of first model doesn't exists
    When I click on '[data-cy="modelizer-switch-button"] [aria-pressed="false"]'
    Then I expect current url is 'modelizer/{{projectName}}/text\?path=terrator-plugin/{{modelName2}}'
